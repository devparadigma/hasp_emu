FROM ubuntu:focal
LABEL description="USB HASP emulator daemon"
...
RUN dpkg --add-architecture i386; \
    ...
    apt-get install -y --no-install-recommends linux-headers-$(uname -r) build-essential automake autoconf libtool libusb-0.1-4:i386 libjansson-dev kmod git bison flex apt-utils libc6-i386; \
    cd /tmp; \
    # ---- Clone vhci_hcd, libusb_vhci, UsbHasp from repositories ----------------------
    git -c http.sslVerify=false clone https://github.com/linuxbuh/vhci_hcd.git; \
    git -c http.sslVerify=false clone https://github.com/linuxbuh/libusb_vhci.git; \
